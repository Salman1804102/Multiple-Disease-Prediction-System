...
if (selected == 'Heart Disease Prediction'):
    ...
    if st.button('Heart Disease Test Result'):
        heart_prediction = heart_model.predict(np.array([[int(age),int(sex),int(cp),int(trestbps),int(chol),int(fbs),int(restecg),int(thalach),int(exang),float(oldpeak),int(slope),int(ca),int(thal)]]))
...
if (selected == "Parkinsons Prediction"):
    ...
    if st.button("Parkinson's Test Result"):
        parkinsons_prediction = parkinson_model.predict(np.array([[fo, fhi, flo, Jitter_percent, Jitter_Abs, RAP, PPQ, DDP, Shimmer, Shimmer_dB, APQ3, APQ5, APQ, DDA, NHR, HNR, RPDE, DFA, spread1, spread2, D2, PPE]]))
